...
def url_photo_maxsize(dict_in_search):
    max_dpi = 0
    elem = 0
    for i in range(len(dict_in_search)):
        ...
        if file_dpi>max_dpi:
            max_dpi = file_dpi
            elem = i
    return dict_in_search[elem].get('url'), dict_in_search[elem].get('type')
...
class Vk_Photo:
    ...
    def _get_photo_info(self):
        ...
        resp_vk = requests.get(url_vk, params = {**self.start_params, **params}).json()['response']
        return resp_vk['count'], resp_vk['items']
    ...
    def _get_logs_only(self):
        ...
        for i in range(photo_count):
            ...
            url_download, picture_size = url_photo_maxsize(photo_items[i]['sizes'])
...
class Yandex:
    ...
    def __init__(self, folder_name, ya_token, num=5):
        self.token = ya_token
...
if __name__ == '__main__':
    ya_token = ''
    user_id_vk = ''
    vk_json = Vk_Photo(TOKEN_Vk,user_id_vk)
    with open ('logs_vk_photo.json','w') as outfile:
        json.dump(vk_json.json, outfile)

    
    my_yandex = Yandex('fotos_from_Vk', ya_token, 5)
    my_yandex.create_copy(vk_json.export_dict)  
